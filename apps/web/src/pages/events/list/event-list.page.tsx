...
export function EventListPage() {
  ...
  const query = useQuery(() => {
    const timing = getValue(form, 'timing');

    return apiQuery('listEvents', {
      query: {
        page: page(),
...
function EventItem(props: { event: EventsListItem }) {
  return (
    <li class="col gap-2">
      ...
      <Link
        href={routes.events.details(props.event.id)}
        class="col gap-4 rounded-lg bg-neutral p-4 shadow-sm"
      >
